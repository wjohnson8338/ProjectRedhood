...
public class GameInputProcessor implements InputProcessor {
    ...
    public GameInputProcessor(GameCamera camera, Player player, PhysicsSystem physicsSystem, AlternatePlayer altPlayer) {
        this.camera = camera;
        this.player = player;
    ...
    public void altProcessInput() {
        // These chains of if statements handle player movement
        if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.W)) {
            if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.A)) {
                altPlayer.movePlayer(Player.MOVE_FORWARDLEFT);
            }
            else if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.D)) {
                altPlayer.movePlayer(Player.MOVE_FORWARDRIGHT);
            }
            else {
                altPlayer.movePlayer(Player.MOVE_FORWARD);
            }
//            System.out.println("Moved Forward");
        }
        else if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.S)) {
            if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.A)) {
                altPlayer.movePlayer(Player.MOVE_BACKLEFT);
            }
            else if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.D)) {
                altPlayer.movePlayer(Player.MOVE_BACKRIGHT);
            }
            else {
                altPlayer.movePlayer(Player.MOVE_BACK);
            }
        }
        else if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.A)) {
            altPlayer.movePlayer(Player.MOVE_LEFT);
        }
        else if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.D)) {
            altPlayer.movePlayer(Player.MOVE_RIGHT);
        }
        
        if (Gdx.input.isKeyPressed(com.badlogic.gdx.Input.Keys.SPACE) && !altPlayer.isStaminaDepleted()) {
            altPlayer.enableAttacking();
        }
